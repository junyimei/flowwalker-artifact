...
__global__ void init_taskassignment(TaskAssignments* assign, int num_walkers,
                                    int block_num) {
  int tid = threadIdx.x;
  int bid = blockIdx.x;
...
__global__ void init_taskassignment_batch(TaskAssignments** assign,
                                          int num_walkers, int block_num,
                                          int batch_size) {
  int tid = threadIdx.x;
  int bid = blockIdx.x;
...
queue_gpu<Task>* get_queuearr(int max_depth, int num_walkers) {
  ...
  CUDA_RT_CALL(
      cudaMalloc(&walker_queue_ptr, (u64)max_depth * sizeof(queue_gpu<Task>)));

  CUDA_RT_CALL(cudaMemcpy(walker_queue_ptr, walker_queue,
                          (u64)max_depth * sizeof(queue_gpu<Task>),
                          cudaMemcpyDefault));
...
queue_gpu<T>* get_queue(int num_walkers) {
  ...
  queue_gpu<T>* walker_queue_ptr =
      get_device_ptr<queue_gpu<T>>(walker_queue, 1);
  delete[] walker_queue;
  return walker_queue_ptr;
}
...
double timing(walker_t* walker_ptr, vtx_t* start_points_ptr,
              ...
              int max_depth, walk_mode type = wb_dynamic) {
  printf("========start timing\n");
#ifdef MICRO_BENCH
  ...
  if (type == queue) {
    ...
    start_time = wtime();
#ifdef MICRO_BENCH
    walker_queue<<<block_num, BLOCK_SIZE>>>(
        walker_ptr, walker_queue_ptr, high_queue_ptr, start_points_ptr,
        result_pool_ptr, num_walkers, d_counter);
#else
    walker_queue<<<block_num, BLOCK_SIZE>>>(walker_ptr, walker_queue_ptr,
                                            high_queue_ptr, start_points_ptr,
    ...
    start_time = wtime();
#ifdef MICRO_BENCH
    walker_twb_dynamic<<<block_num, BLOCK_SIZE>>>(
        walker_ptr, start_points_ptr, start_pointer, result_pool_ptr,
        num_walkers, d_counter);
#else
    walker_twb_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
                                                  start_pointer,
                                                  result_pool_ptr, num_walkers);
#endif
    CUDA_RT_CALL(cudaDeviceSynchronize());
    ...
    start_time = wtime();
#ifdef MICRO_BENCH
    walker_w_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
    ...
    start_time = wtime();
#ifdef MICRO_BENCH
    walker_b_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
                                                start_pointer, result_pool_ptr,
                                                num_walkers, d_counter);
#else
    walker_b_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
    ...
    init_taskassignment<<<block_num, BLOCK_SIZE>>>(assign_ptr, num_walkers,
                                                   block_num);
    CUDA_RT_CALL(cudaDeviceSynchronize());
    if (type == wp) {
      start_time = wtime();
#ifdef MICRO_BENCH
      walker_w<<<block_num, BLOCK_SIZE>>>(
          walker_ptr, start_points_ptr, result_pool_ptr, assign_ptr, d_counter);
#else
      walker_w<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
                                          result_pool_ptr, assign_ptr);
#endif
      CUDA_RT_CALL(cudaDeviceSynchronize());
      total_time = wtime() - start_time;
    } else if (type == wb) {
      start_time = wtime();
#ifdef MICRO_BENCH
      walker_wb<<<block_num, BLOCK_SIZE>>>(
          walker_ptr, start_points_ptr, result_pool_ptr, assign_ptr, d_counter);
#else
      walker_wb<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
                                           result_pool_ptr, assign_ptr);
#endif
      CUDA_RT_CALL(cudaDeviceSynchronize());
      total_time = wtime() - start_time;
      ...
      start_time = wtime();
#ifdef MICRO_BENCH
      walker_twb<<<block_num, BLOCK_SIZE>>>(
          walker_ptr, start_points_ptr, result_pool_ptr, assign_ptr, d_counter);
#else
      walker_twb<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
                                            result_pool_ptr, assign_ptr);
#endif
      CUDA_RT_CALL(cudaDeviceSynchronize());
      total_time = wtime() - start_time;
      ...
      start_time = wtime();
#ifdef MICRO_BENCH
      walker_tw<<<block_num, BLOCK_SIZE>>>(
          walker_ptr, start_points_ptr, result_pool_ptr, assign_ptr, d_counter);
#else
      walker_tw<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
                                           result_pool_ptr, assign_ptr);
#endif
      ...
      CUDA_RT_CALL(
          cudaMalloc(&state_ptr, block_num * BLOCK_SIZE * sizeof(curandState)));
      start_time = wtime();
      ...
      start_time = wtime();
      walker_wb_dprs<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
                                                result_pool_ptr, assign_ptr);
      CUDA_RT_CALL(cudaDeviceSynchronize());
      total_time = wtime() - start_time;
...
#ifdef MICRO_BENCH
  CUDA_RT_CALL(cudaMemcpy(h_counter, d_counter, sizeof(metrics),
    ...
    if (batch_num2 > 0) {
      ...
      if (FLAGS_node2vec) {
        walker_wb_dynamic_dprs<<<block_num, BLOCK_SIZE, 0, streams[1]>>>(
            walker_ptr, b_start_points_ptr, start_pointer + 1,
            b_result_pool_ptr, batch_num2);
      } else {
...
double walk_test(vtx_t*& result_pool_ptr, gpu_graph* graph,
  ...
  if (FLAGS_deepwalk) {
    ...
    total_time = timing<Deepwalk>(walker_ptr, start_points_ptr, result_pool_ptr,
                                  block_num, num_walkers, max_depth, type);
  } else if (FLAGS_ppr) {
    PPR* walker = new PPR(graph_ptr, max_depth, FLAGS_tp);
    PPR* walker_ptr = get_device_ptr<PPR>(walker, 1);
    total_time = timing<PPR>(walker_ptr, start_points_ptr, result_pool_ptr,
                             block_num, num_walkers, max_depth, type);
    ...
    total_time = timing<Node2vec>(walker_ptr, start_points_ptr, result_pool_ptr,
                                  block_num, num_walkers, max_depth, type);
  } else if (FLAGS_metapath) {
    int* schema_ptr = get_device_ptr<int>(schema, schema_len);
    Metapath* walker =
        new Metapath(graph_ptr, max_depth, schema_ptr, schema_len);
    Metapath* walker_ptr = get_device_ptr<Metapath>(walker, 1);
    total_time = timing<Metapath>(walker_ptr, start_points_ptr, result_pool_ptr,
                                  block_num, num_walkers, max_depth, type);
  } else {
...
double walk_batch(vtx_t*& result_pool, gpu_graph* graph,
  ...
  if (FLAGS_deepwalk) {
    ...
    if (FLAGS_syn)
      total_time =
          timing_batch_sync(walker_ptr, start_points, result_pool, batch_size,
                            num_walkers, max_depth, block_num);
    else
      total_time =
          timing_batch_async(walker_ptr, start_points, result_pool, batch_size,
                             num_walkers, max_depth, block_num);
  } else if (FLAGS_ppr) {
    PPR* walker = new PPR(graph_ptr, max_depth, FLAGS_tp);
    PPR* walker_ptr = get_device_ptr<PPR>(walker, 1);
