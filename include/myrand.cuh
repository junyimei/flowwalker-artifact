...
template <int N>
QUALIFIERS void __curand_matvec_inplace_arr(myrandStateArr* state,
                                            unsigned int* matrix) {
  int tid = threadIdx.x;
...
#else
      __curand_matvec_inplace_arr<N>(state,
                                     precalc_xorwow_matrix_host[matrix_num]);
#endif
    }
...
#ifdef __CUDA_ARCH__
      __curand_matvec_inplace_arr<N>(state,
                                     precalc_xorwow_offset_matrix[matrix_num]);
#else
      __curand_matvec_inplace_arr<N>(
          state, precalc_xorwow_offset_matrix_host[matrix_num]);
#endif
    }
...
QUALIFIERS void NextValue(wgrandState* state) {
  state->next_random =
      state->next_random * (unsigned long long)13173779397737131ULL +
...
QUALIFIERS unsigned int myrand(myrandStateArr* state) {
  ...
  state->v[4][tid] =
      (state->v[4][tid] ^ (state->v[4][tid] << 4)) ^ (t ^ (t << 1));
  state->d[tid] += 362437;
  return state->v[4][tid] + state->d[tid];
}

QUALIFIERS unsigned int myrand(curandState* state) { return curand(state); }
...
QUALIFIERS unsigned int myrand(wgrandStateArr* state) {
  ...
  unsigned int ret_value =
      (unsigned int)(state->state_arr[tid].next_random & 0x7fffffffULL);
...
QUALIFIERS void myrand_init(unsigned long long seed,
                            ...
                            unsigned long long offset, myrandState* state) {
...
QUALIFIERS void myrand_init(unsigned long long seed,
                            unsigned long long subsequence,
                            unsigned long long offset, myrandStateArr* state) {
  int tid = threadIdx.x;
  // Break up seed, apply salt
  // Constants are arbitrary nonzero values
  unsigned int s0 = ((unsigned int)seed) ^ 0xaad26b49UL;
...
QUALIFIERS void myrand_init(unsigned long long seed,
                            unsigned long long subsequence,
                            unsigned long long offset, curandStateArr* state) {
  int tid = threadIdx.x;
...
QUALIFIERS void myrand_init(unsigned long long seed,
                            unsigned long long subsequence,
                            unsigned long long offset, wgrandState* state) {
...
QUALIFIERS void myrand_init(unsigned long long seed,
                            unsigned long long subsequence,
                            unsigned long long offset, wgrandStateArr* state) {
  int tid = threadIdx.x;
