...
int main(int argc, char* argv[]) {
  ...
  // if (FLAGS_metapath)
  // {
  ginst = new graph(adj_file.c_str(), edge_file.c_str(), weight_file.c_str(),
                    edgelabel_file.c_str());
  // }
  // else
  // {
  ...
  if (FLAGS_batch) {
    sample_time = walk_batch(result_pool, ggraph, start_points, depth,
                             sample_size, FLAGS_batchsize, schema, schema_len);
  } else {
    ...
    sample_time = walk_test(result_pool, ggraph, start_points, depth,
                            sample_size, mode, schema, schema_len);
  }
