...
__global__ void alias_warp(weight_t *weights, int size, vtx_t *res,
                           int group_size, int granularity,
                           int *task_list, float *prob,
                           vtx_t *alias, vtx_t *large, vtx_t *small)
{
    // int total_threads = blockDim.x * gridDim.x;
    int tid = threadIdx.x;
    int gid = tid + blockDim.x * blockIdx.x;
    ...
    for (int i = worker_id; i < num_tasks; i += gridDim.x * worker_per_blk)
    {
        ...
        selected_id = naive_alias_warp_sampler(local_weights, granularity, &state, local_alias, local_prob, local_large, local_small);
                            ...
                            vtx_t *alias, vtx_t *large, vtx_t *small)
{
    ...
    for (int i = worker_id; i < num_tasks; i += gridDim.x * worker_per_blk)
    {
        ...
        selected_id = naive_alias_block_sampler(local_weights, granularity, &state, local_alias, local_prob, local_large, local_small);
...
__global__ void get_exp(weight_t *weights, int size,
                        int granularity, int *task_list, float *exp)
{
    int tid = threadIdx.x;
    int gid = tid + blockDim.x * blockIdx.x;
...
void test_sampler_helper(weight_t *d_weights, int size, vtx_t *d_res,
                         int num_blocks, int group_size, int sample_type, int granularity)
{
    printf("\n\n---------------------------------------------------------------------------------------------------------\n");
    printf("Test config: {Num of Elements: %d, Number of Thread Blocks: %d, Block Size: %d, "
           "Number of Threads in a Sampler: %d, Sampler Type: %d, Number of Elements Processed "
    ...
    int *d_task_list;
    cudaMalloc((void **)&d_task_list, sizeof(int) * num_tasks);
    ...
    if (sample_type == 0)
    {
        thread_myrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
    }
    else if (sample_type == 1)
    {
#ifdef AVG
        warp_myrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, clk);
#else
        warp_myrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
#endif
    }
    else if (sample_type == 2)
    {
#ifdef AVG
        block_myrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, clk);
#else
        block_myrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
#endif
    }
    else if (sample_type == 3)
    {
        warp_oneloop<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
    }
    else if (sample_type == 4)
    {
        block_oneloop<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
    }
    else if (sample_type == 5)
    {
        its_warp<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, prob);
    }
    else if (sample_type == 6)
    {
        its_block<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, prob);
    }
    else if (sample_type == 7)
    {
        alias_warp<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, prob, alias, large, small);
    }
    else if (sample_type == 8)
    {
        alias_block<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, prob, alias, large, small);
    }
    else if (sample_type == 10)
    {
        thread_curand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
    }
    else if (sample_type == 11)
    {
        warp_curand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
    }
    else if (sample_type == 12)
    {
        block_curand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
    }
    else if (sample_type == 13)
    {
#ifdef AVG
        rjs_warp<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, clk);
#else
        rjs_warp<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
#endif
    }
    else if (sample_type == 14)
    {
...
#ifdef AVG
        rjs_block<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, clk);
#else
        // rjs_block<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, cnt);
        rjs_block<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
#endif
        // cudaDeviceSynchronize();
        ...
        //     cnt_avg += cnt[i];
        // }
        // printf("exp_avg=%.3f, cnt_avg=%.3f\n", (double)exp_avg / num_tasks, (double)cnt_avg / num_tasks);
    }
    else if (sample_type == 20)
    {
        thread_globalrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, state);
    }
    else if (sample_type == 21)
    {
        warp_globalrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, state);
    }
    else if (sample_type == 22)
    {
        block_globalrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, state);
    }
    else if (sample_type == 30)
    {
        thread_globalmyrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, myrand_state);
    }
    else if (sample_type == 31)
    {
        warp_globalmyrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, myrand_state);
    }
    else if (sample_type == 32)
    {
        block_globalmyrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list, myrand_state);
    }
    else if (sample_type == 41)
    {
        warp_wgrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
    }
    else if (sample_type == 42)
    {
        block_wgrand<<<num_blocks, BLOCK_SIZE>>>(d_weights, size, d_res, group_size, granularity, d_task_list);
    }
